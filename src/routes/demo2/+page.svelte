<script lang="ts">
	...
	const chapters = [
		{
			short: 'Opening',
			time: '00:00–00:12',
			note: 'We did not begin with a feature list. We began by asking how businesses actually operate digital assets. That research found a repeatable finance workflow. Privy gave us the authority primitives to make it buildable as Ratib.'
		},
		{
			short: 'Research',
			time: '00:12–00:30',
			note: 'Our first report mapped 27 products. The second compared Ratib with 20 useful benchmarks. The third re-audited what those lessons became inside the product. Each iteration moved from market evidence toward an executable finance workflow.'
		},
		{
			short: 'Landscape',
			time: '00:30–00:48',
			note: 'The market was deep but divided. Finance products handled operations, wallet platforms controlled authority, accounting products proved what happened, and infrastructure platforms connected systems. No single category owned the complete business loop.'
		},
		{
			short: 'Workflow',
			time: '00:48–01:05',
			note: 'Across those different products, the same pattern kept appearing: prepare, review, authorize, settle, reconcile, and report. The category leaders were solving different moments in one shared operating loop.'
		},
		{
			short: 'Question',
			time: '01:05–01:20',
			note: 'The market revealed the same loop repeatedly, but spread across different products. That raised the real design question: could one product connect the complete business workflow without giving itself uncontrolled wallet authority?'
		},
		{
			short: 'Privy',
			time: '01:20–01:42',
			note: 'Privy made that possible. Identity establishes the human actor. Embedded wallets hold and sign. Quorums, policies, authorization signatures, MFA, intents, and events independently enforce what the product is allowed to do.'
		},
		{
			short: 'Architecture',
			time: '01:42–02:00',
			note: 'Ratib authenticates the operator, scopes the organization, prepares the workflow, and explains the route. Privy decides whether the wallet action is authorized. Base Sepolia settles it. Convex records the reconciled business evidence. Ratib roles never become signing rights by accident.'
		},
		{
			short: 'Patterns',
			time: '02:00–02:24',
			note: 'With that authority foundation in place, we adopted proven workflow patterns, never proprietary code. Finance products shaped payroll and payouts. Wallet platforms shaped approvals and policy. Accounting platforms shaped reconciliation and close. Infrastructure platforms shaped reliable developer operations.'
		},
		{
			short: 'Product',
			time: '02:24–02:53',
			note: 'Those patterns now meet inside Ratib. Finance prepares the operation, Privy authorizes it, Base Sepolia settles it, accounting preserves the evidence, and ERPNext and the developer API connect it to the business.'
		},
		{
			short: 'Close',
			time: '02:53–03:00',
			note: 'Privy is not the business workflow. It is what made the business workflow possible. Ratib is the proof: wallet authority can become a coherent B2B finance operating system without giving up the security boundary.'
		}
	] as const;
	...
	const privyPrimitives = [
		...
		},
		{
			...
			copy: 'Wallet creation and use stay inside the product.'
		},
		{
			icon: KeyRound,
			title: 'Owner quorums',
			copy: 'Human owners and thresholds define wallet authority.'
		},
		{
			icon: ShieldCheck,
		...
		{
			icon: LockKeyhole,
			title: 'Signatures + MFA',
			copy: 'Authorization stays with verified human signers.'
		},
		{
			icon: RefreshCcw,
			title: 'Intents + events',
			copy: 'Privileged actions remain observable and reconcilable.'
		}
	] as const;
	...
	const patternGroups = [
		{
			index: '01',
			companies: 'Request + Mural',
			pattern: 'Payroll + payouts',
			copy: 'Periods, batches, review, scheduling, and partial outcomes.'
		},
		{
			index: '02',
			companies: 'Safe + Fireblocks + Fordefi',
			pattern: 'Authority controls',
			copy: 'Proposals, policy, quorum, and transaction review.'
		},
		{
			index: '03',
			companies: 'Bitwave + Cryptio',
			pattern: 'Accounting + close',
			copy: 'Journals, reconciliation, exceptions, and review.'
		},
		{
			index: '04',
			companies: 'Modern Treasury + Dfns',
			pattern: 'Developer reliability',
			copy: 'Identity, idempotency, request authorization, and events.'
		}
	] as const;
...
<svelte:head>
	<title>Ratib | Privy made it buildable</title>
	<meta
		name="description"
		content="A three-minute story about the market research, Privy authority, and the workflow patterns that became Ratib."
	/>
...
<nav class="chapter-rail" aria-label="Presentation chapters">
	<p>
		<strong>{String(activeIndex + 1).padStart(2, '0')}</strong> / {String(chapters.length).padStart(
			2,
			'0'
		)}
	</p>
...
<main class="demo-deck" aria-label="Ratib three-minute product presentation">
	...
	>
		...
		<div class="opening-copy reveal">
			<p class="kicker"><Sparkles size={14} /> THREE REPORTS · ONE PRODUCT STORY</p>
			<h1 id="opening-title">
				Research found the workflow.<br /><em>Privy made it buildable.</em>
			</h1>
			<p class="lede">How market evidence became a governed B2B finance operating system.</p>
		</div>
		<div class="opening-ticket reveal delay-2">
			<span>RATIB / PRIVY STORY</span>
	...
	>
		...
		<div class="problem-copy reveal">
			<p class="kicker">THE FIRST RESEARCH</p>
			<h2 id="problem-title">We mapped the system<br /><em>before building the product.</em></h2>
			<p>Each report turned market evidence into a sharper product decision.</p>
		</div>
		<div class="system-route" aria-label="Ratib research progression">
			<article class="system-card wallet reveal delay-1">
				<div><WalletCards size={22} /><span>01</span></div>
				<small>LANDSCAPE</small>
				<h3>27 products</h3>
				<p>The complete digital-asset operations market.</p>
			</article>
			...
			<article class="system-card workflow reveal delay-2">
				...
				<h3>20 benchmarks</h3>
				<p>Ratib measured against category leaders.</p>
			</article>
			...
			<article class="system-card evidence reveal delay-3">
				...
				<h3>Product audit</h3>
				<p>Research lessons traced into working Ratib flows.</p>
			</article>
		</div>
		<p class="problem-foot reveal delay-3">
			<CircleDot size={13} /> Evidence first. Product decisions second.
		</p>
	</section>
	...
	>
		...
		<div class="market-intro reveal">
			<div>
				<p class="kicker light">THE ORIGINAL LANDSCAPE</p>
				<h2 id="market-title">
					The winners own different moments.<br /><em>No one owns the whole loop.</em>
				</h2>
			</div>
			<div class="research-stats" aria-label="Market research totals">
				...
				<p><strong>03</strong><span>research iterations</span></p>
			</div>
		</div>
		...
		<p class="method-note">
			Finance prepares · wallets authorize · networks settle · accounting proves.
		</p>
	</section>
	...
	>
		<div class="gap-copy reveal">
			<p class="kicker">WHAT THE MARKET TAUGHT US</p>
			<h2 id="gap-title">One repeatable loop.<br /><em>Six governed moments.</em></h2>
			<p>
				Prepare, review, authorize, settle, reconcile, and report without losing the authority
				boundary.
			</p>
		</div>
		<div class="gap-map" aria-label="Ratib connects the finance operating loop">
			...
			<div class="gap-center reveal delay-2">
				<span>THE OPERATING LAYER</span><strong>RATIB</strong><small
					>Privy-backed finance operations</small
				>
			</div>
			{#each ['Prepare', 'Review', 'Authorize', 'Settle', 'Reconcile', 'Report'] as item, index}
		...
		<div class="gap-thesis reveal delay-3">
			<span>SETTLEMENT CONNECTS THE LOOP</span>
			<p>One governed context around <em>every transaction.</em></p>
		</div>
	</section>

	<section
		id="question"
		class="slide question-slide"
		class:active-slide={activeIndex === 4}
		data-slide="4"
		aria-labelledby="question-title"
	>
		<div class="question-number" aria-hidden="true">?</div>
		<div class="question-copy reveal">
			<p class="kicker">THE QUESTION</p>
			<h2 id="question-title">
				What if one product connected<br /><em>the whole financial loop?</em>
			</h2>
			<p>Without making the product itself the wallet authority.</p>
		</div>
		<div class="question-tension" aria-label="The product and authority design question">
			<article class="reveal delay-1">
				<div><Layers3 size={22} /><span>BUSINESS</span></div>
				<strong>One finance workflow</strong>
				<small>Prepare · review · reconcile · report</small>
			</article>
			<div class="question-gap reveal delay-2" aria-hidden="true">
				<i></i><b>?</b><i></i>
			</div>
			<article class="authority reveal delay-3">
				<div><LockKeyhole size={22} /><span>AUTHORITY</span></div>
				<strong>Independent wallet control</strong>
				<small>Authorize · sign · enforce · settle</small>
			</article>
		</div>
		<p class="question-foot reveal delay-3">
			The workflow can coordinate authority. It cannot invent it.
		</p>
	</section>
	...
	>
		<div class="privy-heading reveal">
			<div>
				<p class="kicker light">THE ANSWER</p>
				<h2 id="privy-title">Privy made that<br /><em>architecture possible.</em></h2>
			</div>
			<p>
				Verified people, embedded wallets, policy, quorum, MFA, intents, and events gave the
				business workflow a secure foundation.
			</p>
		</div>
		...
		<div class="privy-answer reveal delay-3">
			<ShieldCheck size={24} />
			<p>
				Privy made the <strong>B2B operating layer</strong> possible without weakening wallet authority.
			</p>
		</div>
	</section>
	...
	>
		<img
			...
			aria-hidden="true"
		/>
		<div class="patterns-heading reveal">
			<p class="kicker">PROVEN PATTERNS ADOPTED</p>
			<h2 id="patterns-title">We borrowed the workflow.<br /><em>Never the code.</em></h2>
			<p>Each precedent maps directly to a capability Ratib assembled around Privy.</p>
		</div>
		<div class="pattern-paths" aria-label="Competitor workflow precedents adopted in Ratib">
			{#each patternGroups as group, index}
				<article class="pattern-path reveal" style={`--delay: ${0.1 + index * 0.08}s`}>
					<div class="pattern-source">
						<span>{group.index} / PRECEDENT</span>
						<strong>{group.companies}</strong>
					</div>
					<div class="pattern-arrow" aria-hidden="true"><i></i><ArrowRight size={16} /></div>
					<div class="pattern-result">
						<span>RATIB ANALOGUE</span>
		...
		<div class="patterns-hub reveal delay-3">
			<span>ASSEMBLED IN</span><strong>RATIB</strong><small>One Privy-backed operating loop</small>
		</div>
	</section>
	...
	>
		...
		<div class="product-copy reveal">
			<p class="kicker">RATIB TODAY</p>
			<h2 id="product-title">Those patterns became<br /><em>one business workflow.</em></h2>
			<p>Finance prepares. Privy authorizes. Ratib preserves the operational evidence.</p>
			<div class="proof-list">
				{#each ['Payroll + payouts', 'Embedded approvals', 'Immutable accounting', 'ERPNext sync', 'Developer API', 'Reconciliation'] as proof}
		...
		<div class="workspace-preview reveal delay-2" aria-label="Illustrative Ratib workspace preview">
			<aside>
				...
				<p class="selected"><Layers3 size={13} /> Payroll</p>
				<p><WalletCards size={13} /> Wallets</p>
				<p><Route size={13} /> Payouts</p>
				<p><ShieldCheck size={13} /> Approvals</p>
				<p><FileCheck2 size={13} /> Accounting</p>
			</aside>
			<div class="workspace-main">
				<header>
					<div><small>RATIB TODAY</small><strong>One business workflow</strong></div>
				...
				<div class="operation-list">
					...
					<p>
						<i class="coral"></i><span
							><b>Payroll run</b><small>Prepared and awaiting Privy authority</small></span
						><em>Pending approval</em>
					</p>
					<p>
						<i class="lime"></i><span
							><b>ERP synchronization</b><small>Accounting evidence connected</small></span
						><em class="success">Reconciled</em>
					</p>
					<p>
						<i class="blue"></i><span
							><b>API-created payout</b><small>Human wallet authority retained</small></span
	...
	<section
		...
		class:active-slide={activeIndex === 9}
		data-slide="9"
	...
	>
		...
		<div class="close-copy reveal">
			<p class="kicker light">THE B2B PROOF</p>
			<h2 id="close-title">Privy made the B2B<br /><em>product possible.</em></h2>
			<p>Ratib turns wallet authority into a coherent finance operating system.</p>
...
<style>
	...
	.question-slide {
		...
		background:
			radial-gradient(circle at 16% 18%, rgba(71, 121, 232, 0.16), transparent 28%),
			linear-gradient(125deg, #fff9ef 0%, #edf1d5 100%);
	}
	.question-number {
		position: absolute;
		right: 11vw;
		top: 4vh;
		color: rgba(7, 63, 67, 0.045);
		font-family: Georgia, serif;
		font-size: 70vh;
		line-height: 1;
	}
	...
	.question-copy .kicker {
		justify-content: center;
	}
	.question-copy h2 {
		margin-top: 18px;
		font-size: clamp(3.7rem, 5.35vw, 6.35rem);
		line-height: 0.92;
	}
	.question-copy h2 em {
		color: var(--coral);
	...
	.question-tension article {
		...
		box-shadow: 0 18px 45px rgba(7, 63, 67, 0.1);
		backdrop-filter: blur(9px);
	}
	.question-tension article.authority {
		border-color: rgba(47, 123, 103, 0.45);
		background: rgba(229, 241, 218, 0.9);
	}
	.question-tension article > div {
		display: flex;
		align-items: center;
		gap: 8px;
		color: var(--green);
	}
	.question-tension article > div span {
		font-size: 0.52rem;
		font-weight: 900;
		letter-spacing: 0.12em;
	}
	.question-tension article strong {
		margin-top: 13px;
		font-family: Georgia, serif;
		font-size: 1.35rem;
		font-weight: 500;
	}
	.question-tension article small {
		margin-top: 6px;
		color: #70847f;
		font-size: 0.58rem;
	}
	...
	.patterns-heading h2 em {
		color: var(--coral);
	}
	.patterns-heading > p:last-child {
		max-width: 470px;
		margin-top: 23px;
		color: #607671;
		line-height: 1.65;
	...
	.pattern-path {
		...
		align-items: stretch;
		overflow: hidden;
		border: 1px solid rgba(7, 63, 67, 0.18);
		border-radius: 10px;
		background: rgba(255, 253, 248, 0.82);
		box-shadow: 0 15px 35px rgba(7, 63, 67, 0.08);
		backdrop-filter: blur(10px);
	}
	.pattern-source,
	.pattern-result {
	...
	.pattern-result {
		border-left: 1px solid rgba(7, 63, 67, 0.1);
		background: rgba(229, 241, 218, 0.58);
	}
	.pattern-result span {
		color: #3e806e;
	}
	...
	.pattern-arrow {
		...
		gap: 4px;
		color: var(--coral);
		background: rgba(246, 238, 224, 0.76);
	}
	.pattern-arrow i {
		width: 20px;
		border-top: 1px dashed var(--coral);
